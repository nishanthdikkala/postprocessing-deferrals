...
#raw_data <- read.csv("./cox-parsed.csv")
...
# fig1 <- barplot(black_score_pdf, main="PDF of decile scores for African-Americans",
#         xlab="Decile Score", ylab="Fraction",ylim=c(0,0.3), names.arg=1:10)
# fig2 <- barplot(white_score_pdf, main ="PDF of decile scores for Caucasians",
#         xlab="Decile Score", ylab="Fraction", ylim=c(0,0.3), names.arg=1:10)

score_pdfs = rbind(black_score_pdf,white_score_pdf)
# fig3 <- barplot(score_pdfs, beside = T, main ="PDF of decile scores for Caucasians",
#         xlab="Decile Score", ylab="Fraction", ylim=c(0,0.3), names.arg=1:10)
...
# fig4 <- barplot(black_deferrals, main="Number of deferrals for African-Americans by the MIN-PDF method",
#           xlab="Decile Score", ylab="Number of deferrals", names.arg=1:10)
# text(fig4, black_deferrals, labels = format(black_deferrals, 10),pos = 3, cex = .75)
# fig5 <- barplot(white_deferrals, main="Number of deferals for Caucasians by the MIN-PDF method",
#           xlab="Decile Score", ylab="Number of deferrals", names.arg=1:10)
# text(fig5, white_deferrals, labels = format(white_deferrals, 10),pos = 3, cex = .75)


deferrals = rbind(black_deferrals,white_deferrals)
# fig6 <- barplot(deferrals, beside=T, main="Number of deferals across groups by the MIN-PDF method",
#           xlab="Decile Score", ylab="Number of deferrals", names.arg=1:10)

deferrals_frac = rbind(black_deferrals/black_score_freq, white_deferrals/white_score_freq)
# fig7 <- barplot(deferrals_frac, beside=T, main="Fraction of deferals across groups by the MIN-PDF method",
#           xlab="Decile Score", ylab="Number of deferrals", names.arg=1:10)
...
# bar1 = t(matrix(c(black_nondeferrals,black_deferrals), nrow=10))
# bar2 = t(matrix(c(white_nondeferrals,white_deferrals), nrow=10))
# bars = rbind(bar1, bar2)
# barplot(bars, beside=T, main="",xlab="Decile Score", ylab="", names.arg=1:10)
col_black = "gray38"
...
barplot(dat, space=c(0.5,0.), col=c(col_black,col_black_deferral,col_white,col_white_deferral), ylim = c(0,0.3),
        ...
        main="Equalizing DOCS using the minimum method.\n Total deferral rate in each group = 24.5%",
...
#                   geom_bar() + xlab("Decile Score") + ylim(0, 650) + ggtitle("Black Defendant's Decile Scores")
# pwhite <- ggplot(data=filter(df, race =="Caucasian"), aes(ordered(decile_score))) +
#                   geom_bar() + xlab("Decile Score") + ylim(0, 650) + ggtitle("White Defendant's Decile Scores")
